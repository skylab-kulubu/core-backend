...
func ConfigFromEnv(getenv func(string) string, jwtIssuer string) (Config, error) {
	...
	config.RequiredReplicas, err = requiredNonNegativeInt(getenv, "ACCOUNT_ACCESS_REDIS_REQUIRED_REPLICAS")
	if err != nil {
		return Config{}, err
	}
	...
	config.WaitTimeout, err = requiredPositiveDuration(getenv, "ACCOUNT_ACCESS_REDIS_WAIT_TIMEOUT")
	if err != nil {
		return Config{}, err
	}
	return config, nil
...
func NewRedisClient(config Config) (*redis.Client, error) {
	...
	return redis.NewClient(&redis.Options{
		...
		DB: config.DB, TLSConfig: tlsConfig,
		DialTimeout: config.OperationTimeout, ReadTimeout: config.OperationTimeout, WriteTimeout: config.OperationTimeout,
